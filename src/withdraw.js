...
export default function Withdraw(){
    ...
    useEffect(() => {
        async function fetchdata() {
            await axios.get('https://servers-0ihb.onrender.com/data').then((item) => {
             setData(item.data);
             console.log(data);
    ...
    async function submitHandler(e){    
        ...
        if(isNaN(withdraw) || withdraw < 0){
            ...
            await axios.put(`https://servers-0ihb.onrender.com/update/${id}`,{amount:total-withdraw}).then( setTotal(total - withdraw)).then(alert(`amount ${withdraw} is withdrawn successfully !`));
        }
    }
